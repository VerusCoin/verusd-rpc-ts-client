...
function normalizeConverterCandidate(record) {
    ...
    const currencyids = Object.keys(record).filter(key => {
        if (converterMetadataKeys.has(key))
            return false;
...
class VerusdRpcInterface {
    constructor(chain, baseURL, config, rpcRequest, APIAuth) {
        ...
        this.listcurrencystatescache = new Map();
        this.infocache = null;
        if (rpcRequest)
    ...
    getIdentityContent(...args) {
        return this.request(new verus_typescript_primitives_1.GetIdentityContentRequest(this.chain, ...args));
    }
    getIdentitiesWithAddress(...args) {
        return this.request(new verus_typescript_primitives_1.GetIdentitiesWithAddressRequest(this.chain, ...args));
    }
    getCurrency(...args) {
    ...
    signRawTransaction(...args) {
        return this.request(new verus_typescript_primitives_1.SignRawTransactionRequest(this.chain, ...args));
    }
    sendCurrency(...args) {
    ...
    zGetOperationStatus(...args) {
        return this.request(new verus_typescript_primitives_1.ZGetOperationStatusRequest(this.chain, ...args));
    }
    signData(...args) {
        return this.request(new verus_typescript_primitives_1.SignDataRequest(this.chain, ...args));
    }
    updateIdentity(...args) {
        return this.request(new verus_typescript_primitives_1.UpdateIdentityRequest(this.chain, ...args));
    }
    ...
    getCachedCurrencyState(definition) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            if (definition.bestcurrencystate) {
                return { definition, state: definition.bestcurrencystate };
            }
            const cachedDefinition = (_a = this.currencycache.get(definition.currencyid)) === null || _a === void 0 ? void 0 : _a.result;
            if (cachedDefinition === null || cachedDefinition === void 0 ? void 0 : cachedDefinition.bestcurrencystate) {
            ...
            const response = yield this.request(new verus_typescript_primitives_1.GetCurrencyRequest(this.chain, definition.currencyid));
            if (response.error)
    ...
    getConverterExportCurrencyId(definition) {
    ...
    getCachedCurrencyConverters(...args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.converterscache.has(key)) {
                ...
                if (response.error)
                    return response;
                const normalizedResponse = Object.assign(Object.assign({}, response), { result: response.result.map(normalizeConverterCandidate) });
    ...
    getCurrencyConversionPathsRec(src_1, dest_1) {
        return __awaiter(this, arguments, void 0, function* (src, dest, includeVia = true, ignoreCurrencies = [], via, root) {
            const currChainDefinition = (VerusdRpcInterface.extractRpcResult(yield this.getCachedCurrency(this.chain)));
            const gatewayConverterSource = (0, flags_1.checkFlag)(src.options, flags_1.IS_GATEWAY_CONVERTER_FLAG);
            ...
            const converterResponse = yield this.getCachedCurrencyConverters(dest == null ? [src.currencyid] : [src.currencyid, dest.currencyid]);
            if (converterResponse.error)
                throw new Error(converterResponse.error.message);
            ...
            destination_iterator: for (const path of paths) {
                let fullCurrencyDefinition = (VerusdRpcInterface.extractRpcResult(yield this.getCachedCurrency(path.currencyid)));
                let pricingCurrencyState;
                let price;
                let viapriceinroot;
                let destpriceinvia;
                if (via) {
                    const pricingCurrency = yield this.getCachedCurrencyState(via);
                    ...
                    const pricingCurrency = yield this.getCachedCurrencyState(fullCurrencyDefinition);
                ...
                for (const reserve of src.currencies) {
                    ...
                    if (!ignoreCurrencies.includes(reserve)) {
                        if (via) {
                            ...
                            const pricingCurrency = yield this.getCachedCurrencyState(src);
    ...
    _getCurrencyConversionPaths(src_1, dest_1) {
        return __awaiter(this, arguments, void 0, function* (src, dest, includeVia = true, ignoreCurrencies = [], via, root) {
    ...
    getCurrencyConversionPaths(src_1, dest_1) {
        return __awaiter(this, arguments, void 0, function* (src, dest, includeVia = true, ignoreCurrencies = [], via, root) {
            var _a;
            try {
                ...
                for (const currency of allCurrencies) {
                    ...
                    if (currency.bestheight == null)
                        continue;
                    // A list entry without state must not replace a supplied pricing state.
                    if (currency.bestcurrencystate == null &&
                        ((_a = this.currencycache.get(currency.currencydefinition.currencyid)) === null || _a === void 0 ? void 0 : _a.result.bestcurrencystate) != null) {
                        continue;
                    }
                    const definition = Object.assign(Object.assign({}, currency.currencydefinition), { bestheight: currency.bestheight, bestcurrencystate: currency.bestcurrencystate });
